...
def extract_feature(checkpoint1, checkpoint2, x, y):
    ...
    :param y: the GroundTruth input tensor with size of [batch_size,channels,width,height]==>[batch_size,3,256,256]
...
def computeLoss(pred_predict, prj_predict):
    ...
    :param prj_predict: the GroundTruth data feed to the VGG model and produce the prediction ==> [[batch_size,256,64,64][batch_size_512,16,16]]
...
def perceptualLoss(prediction_batch, prj_train_batch):
    ...
    :param prj_train_batch: the GroundTruth pictures with shape of [batch_size,3,256,256]
    :return:
    """
    ckp1, ckp2 = get_model()
